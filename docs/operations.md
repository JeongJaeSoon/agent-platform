...
compose의 `apps` profile에서는 `reconciler` 서비스가 이 pass를 기본으로 반복 실행한다(94S-320). `apps/control-host/src/reconciler/loop.ts`가 pass마다 `main.ts`를 자식 프로세스로 띄우므로 앱의 one-shot 계약은 그대로다. pass는 겹치지 않고 직렬로 돈다.
