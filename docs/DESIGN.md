...
**quiescence 판정과 checkpoint lease 규칙 (94S-208, 2026-09-23).** 위 문단의 "quiesce 확인"과 "배타적 checkpoint lease"를 다음 계약으로 고정한다. 94S-246의 publisher는 이 계약 위에 선다.

- **판정 근거.** 엔진이 스스로 아는 것만 증명한다. SDK `PreToolUse` hook은 main thread와 subagent의 모든 tool 실행 전에 불리고 CLI는 그 답을 기다린다. 여기서 받아들인 tool은 `PostToolUse`·`PostToolUseFailure`·`PermissionDenied` hook이나 해당 `tool_result`, 또는 `canUseTool`의 거절로 끝날 때까지 in flight다(`tool_in_flight`). 열려 있는 `canUseTool` callback도 같다. background 작업은 SDK의 `background_tasks_changed` level signal(replace semantics)로 본다. `ambient` task도 센다 — `ambient`는 "활동 표시에서 제외"라는 뜻일 뿐 쓰지 않는다는 보장이 아니다(`background_writer`). stream이 끝나도 tool·task를 정리하지 않는다. stream 종료는 엔진이나 그 자식의 종료를 증명하지 않으므로 그 run은 checkpoint에 부적합한 채로 남는다.
...
- **직렬화 가능한 MCP 서버(stdio/sse/http).** 구성을 통째로 해시하되 `headers`·`env`는 **키 이름 집합만** 해시한다. 값만 바뀐 것은 로테이션이고, 키가 늘거나 줄면 도구 표면이 바뀐 것이다. 다만 두 컨테이너에는 자격 증명만 있는 것이 아니다 — 환경 변수 `TENANT_ID`나 헤더 `X-Tenant`처럼 값이 tenant 결속이나 모드를 뜻할 수 있고 키 이름은 그것을 못 본다. 그래서 `headers`·`env`에 값이 하나라도 있는 서버는 `identities.mcpServers[name]`(그 값들이 뜻하는 비밀 아닌 것의 호출자 진술)을 **추가로** 요구하고 함께 해시한다(Codex astra 판정, 8차 제안의 키 이름 규칙에 이 조건을 더함). `url`·`command`·`args`는 값까지 해시한다 — 같은 이름을 다른 곳으로 돌리면 다른 checkpoint이고, 거기 든 비밀은 로테이션 시 digest를 바꾼다(지원하는 자격 증명 위치는 `profile.auth`·`headers`·`env` 셋뿐).
- **in-process MCP 서버(`{type:"sdk", instance}`)와 local plugin.** 값으로 해시할 수 없으므로 호출자가 `identities.mcpServers[name]`·`identities.plugins[path]`로 명시한 identity(구현·설정 revision과 tenant 결속을 덮고 로테이션되는 자격 증명은 뺀 digest 문자열)를 대신 해시한다. sdk 항목의 `name`·`type` 같은 plain 필드는 유지한다. identity는 측정이 아니라 호출자의 주장이라 내용을 바꾸고 identity를 유지하면 여기서는 못 잡는다 — 그것은 identity를 주는 쪽의 책임이다. SDK의 `name`·`version`에서 유추하지 않는다(SDK 구성은 version을 노출하지 않고, 같은 버전이 다른 tenant 상태를 들 수 있다). identity가 없으면 `validateRuntimeConfig`가 new·resume 모두 **시작 전에** `UnidentifiedComponentError`(`reason: "unidentified_component"`, component·name 포함)로 거절한다. 시작하지 못한 run은 checkpoint를 만들지 않으므로 별도 `CheckpointBlockReason`은 두지 않는다.
...
- **push 대신 bundle.** 1번의 "remote에 push"는 하지 않는다. workspace는 임시 index 사본에 `add -u`한 tree를 HEAD 위 commit으로 만들어(checkout의 HEAD·index·branch는 건드리지 않는다) 그 commit과 HEAD·branch ref를 git bundle 하나에 담는다. push는 사용자 branch 이력에 checkpoint commit을 남기고 credential을 워커 쓰기 경로에 들이기 때문이다. untracked 파일은 bundle 밖 객체(`untracked/<sha256>`)로 따로 올리고 manifest가 경로·실행 비트와 함께 가리킨다. capture는 checkout 자신의 config가 아니라 복원이 쓰는 설정(`core.attributesFile=/dev/null`, `core.autocrlf=false`, `core.eol=lf`, `core.fileMode=true`, `core.ignoreCase=false`, `core.symlinks=true`, `core.safecrlf=true`, `core.useReplaceRefs=false`, `core.checkStat=default`, `core.trustctime=true`)으로 돈다. replace ref는 workspace에만 남으므로, 그것을 거쳐 읽은 HEAD의 tree는 bundle의 commit에 없는 tree다. staging의 `GIT_DIR`도 scratch 저장소라서 workspace 저장소에만 있는 `info/attributes`·local config는 담기는 바이트를 바꾸지 못한다 — 복원에는 둘 다 없다. 그래서 tracked 파일의 mode 변경, tracked symlink를 대신한 일반 파일, 대소문자만 다른 untracked 파일이 그대로 담긴다. tracked 파일은 stat을 믿지 않고 다시 읽는다(`add --renormalize`). 디스크의 줄바꿈이 복원이 쓸 줄바꿈과 다르면 거절한다 — 엔진 쪽 `core.eol`·`autocrlf`로 쓰였거나 `.gitattributes`가 나중에 바뀐 파일은 blob으로는 조용히 정규화되기 때문이다. 파일 이름이 UTF-8이 아니면 tracked·untracked 모두 거절한다(BOM도 이름의 일부로 둔다). git 출력·bundle이 상한을 넘으면 git을 죽이고 거절한다. index는 상한 안에서 복사한 사본만 git에 읽히고, index가 없으면 빈 index로 보지 않고 거절한다(빈 index에 staging하면 ignore 규칙에 걸리는 tracked 파일이 빠진다). staging이 scratch에 쓸 tracked 변경의 크기도 먼저 잰다. 바뀐 파일은 ctime까지 보는 stat으로 고르므로, 옛 크기와 mtime으로 되돌려 쓴 파일도 센다. 바뀐 symlink는 대상 경로의 길이를 센다. 다만 index가 갱신된 그 초 안에 다시 쓴 파일은 놓친다(git은 USE_NSEC 없이 ctime을 초 단위로만 둔다). 그렇게 놓친 파일이 staging에 쓰는 양은 workspace quota 안의 tracked 바이트에 묶인다. `.gitattributes`가 바뀌었으면 `--renormalize`가 어떤 tracked 파일이든 다시 쓸 수 있으므로 전부를 센다. `ident` 속성이 있는 파일이 바뀌었으면 거절한다 — 복원이 `$Id$`를 새 blob id로 다시 펼쳐 바이트가 달라지기 때문이다. git이 끝난 뒤에는 파이프를 잠깐만 더 읽는다. 죽은 git이 남긴 helper가 파이프를 쥐고 있어도 capture가 멈추지 않게 하려는 것이다(process group kill은 94S-289). ignored 파일은 담지 않는다 — 재개 후 다시 만들어지는 산출물(`node_modules`, build 결과)이고, 담으면 크기 상한이 사실상 사용자 `.gitignore`에 달린다.
...
- **content-addressed 업로드는 요청 하나 (94S-380, 2026-09-25).** bundle·untracked·transcript 합본처럼 key가 내용의 digest인 객체는 쓰기 전 존재 확인 GET 없이 create-only PUT 하나만 보낸다(`putImmutable`의 `contentAddressed`). 그 GET은 `If-None-Match`를 무시하는 endpoint에서 동시 창 바깥의 덮어쓰기를 막는 보조 장치다. key가 digest면 덮어써도 같은 바이트라 막을 것이 없다. `duplicate`·`conflict` 판정은 412 뒤의 읽기가 그대로 한다. 그래서 untracked 파일 N개의 업로드가 route 요청 2N에서 N으로 준다. manifest와 transcript slot part는 전과 같이 먼저 읽는다. 요청 자체를 없애는 두 방안은 기각했다. 이전 publish가 올린 같은 digest 객체를 재사용하면 finalize의 디렉터리 격리 규칙(manifest는 자기 publish 디렉터리 밖을 가리키지 않는다)을 깬다. checkpoint GC가 그 규칙에 기대어 디렉터리를 회수한다. untracked를 객체 하나로 묶으면 manifest 형식, finalize 검증, restore가 모두 바뀐다.
...
- **단계별 소요 시간 (94S-380, 2026-09-25).** `worker.checkpoint.published`와 `worker.checkpoint.failed`는 `durations_ms`에 단계별 ms를, `total_ms`에 checkpoint 요청부터의 전체 ms를 싣는다. 단계는 `request`(checkpoint 요청), `chain`(이전 bundle 존재 확인), `git`(workspace capture·bundle 생성), `bundle_upload`, `untracked_upload`, `transcript`(transcript pin·병합), `recheck`(mirror 재확인), `manifest`(manifest PUT)다. 실패한 publish는 도달한 단계까지만 싣는다. 예산(interrupt 뒤 30초, 94S-382)을 넘겨 버려진 capture도 끝나면 같은 줄을 남긴다.
- **실패는 turn 실패가 아니다.** publish가 도중에 실패하면 `worker.checkpoint.failed`(stage·reason·revision·manifest_ref·`durations_ms`·`total_ms`)를 남기고 checkpoint 없이 finalize한다. 소유권 상실만 던진다. checkpoint를 실은 finalize가 결정적 `CHECKPOINT_UNAVAILABLE`(manifest 검증 거절)을 받고 앞서 결말 모를 시도가 없었으면, 같은 finalize key로 checkpoint 없이 한 번 더 finalize한다. 거절된 요청은 아무것도 commit하지 않았기 때문이다.
...
**transcript 병합과 크기 상한 (94S-314, 94S-296, 2026-09-24).** mirror는 append마다 write-once part를 하나씩 만들고, 재개된 generation은 이전 part를 그대로 상속한다. 그대로 두면 manifest가 가리키는 객체 수가 세션 수명에 비례해 늘어 객체 상한(20,000)에 닿는다. transcript를 한꺼번에 들고 있는 워커 메모리에도 상한이 없었다.

- **병합.** publish의 capture는 transcript 하나가 part 512개를 넘기거나 세션 전체가 4,096개를 넘기면, 그 transcript가 pin할 part를 순서대로 4 MiB 이하 run으로 묶어 합본 part(`merged-<sha256>.jsonl`)로 다시 쓴다. 한 개짜리 run은 그대로 둔다. 합본은 원본 바이트를 순서대로 이어 붙인 것이라 uuid dedup, 같은 uuid에 다른 본문이면 거절, digest 검증이 원본과 똑같이 적용된다. 합본은 이번 generation prefix 아래에 create-only로만 쓴다. key가 digest라서 재시도도 같은 key에 같은 바이트를 쓴다. 이후 capture는 합본을 이어받고 그 뒤에 쓴 part만 붙인다. 합본 쓰기가 실패하면 publish는 `publish_failed`로 끝나고, store는 이전 part 목록을 그대로 쓴다. 엔진의 `load`는 합본을 보지 않는다. 그래서 manifest 객체 수는 세션 길이가 아니라 transcript 바이트에 비례하고, 바이트는 아래 상한에 묶인다. 대신 한 generation이 오래 살면 대체된 합본이 그 generation이 끝날 때까지 남는다. 회수는 94S-326이 맡는다.
- **상한.** part 하나는 16 MiB, transcript 전체(root와 모든 subagent의 합)는 64 MiB다(`MAX_TRANSCRIPT_PART_BYTES`, `MAX_TRANSCRIPT_BYTES`, runtime-core). 워커 publish, finalize 검증, restore(서버 restore plan과 워커의 manifest 검사)가 모두 이 값 하나를 쓴다. 판정은 ref가 주장하는 `bytes`로 한다. 모든 reader가 저장된 바이트를 그 값에 맞춰 보므로, 어떤 part도 받기 전에 거절할 수 있다. 값은 워커 메모리(`WORKER_MEMORY_MB`, 기본 2048)에 맞췄다. 엔진은 resume할 때 transcript 전체를 읽고, 워커도 그 전에 검증하며 바이트를 들고 있다.
- **상한에 걸린 세션.** 새 checkpoint를 막는다. compaction은 요구하지 않는다. 엔진 transcript는 append-only이고, 플랫폼이 줄일 방법이 없다. 워커는 `publish_failed`(stage `transcript`)를 advisory로 남기고 세션은 계속 돈다. 마지막 checkpoint가 재개 지점으로 남는다. 워커가 교체되면 94S-288 gap gate가 세션을 `recovery_required`로 보내고, 이어 가는 길은 `start_fresh`다. 상한을 낮추면 이미 커밋된 checkpoint 중 넘는 것도 복원이 거절된다. 객체 수 상한과 같은 규칙이다.
- **capture 비용 (94S-380, 2026-09-25).** capture는 transcript마다 지난 capture가 센 entry 수와 uuid dedup 상태를 기억하고, 그 뒤에 붙은 part만 파싱한다. 이번 generation이 쓴 part의 sha256도 쓸 때 한 번만 계산한다. 병합은 원본을 순서대로 이어 붙인 것이라 센 상태를 합본으로 그대로 옮긴다. part 목록이 기억한 목록을 앞부분으로 이어 가지 않으면 처음부터 다시 센다. 파싱이나 uuid 충돌로 실패하면 기억을 버리므로 다음 capture도 같은 이유로 거절한다. `entryCount`와 digest의 의미는 그대로다.
- **복원 메모리.** `verifyInherited`는 part 하나씩 파싱해 검증하고 entry는 버린다. uuid 중복 판정은 본문 대신 본문의 digest를 기억한다. 받은 바이트는 엔진의 첫 `load`에 쓰려고 들고 있는다.
...
이전 generation으로 돌아가는 조건은 좁다(94S-204). pointer의 checkpoint가 **손상**됐을 때만, 즉 객체가 사라졌거나 기록한 바이트와 달라졌을 때만 그 checkpoint가 딛고 선 이전 checkpoint들을 parent 사슬(`checkpoints.parent_revision`)을 따라 최대 `maxRestoreFallbacks`개(기본 3, 상한 10) 다시 해시해 본다. 폴백 뒤의 커밋은 복원한 revision을 parent로 기록한다. 그래서 그때 건너뛴 revision은 나중에 멀쩡해 보여도 다시 후보가 되지 않는다. 그 revision은 세션이 버린 이력이기 때문이다. 한도를 낮췄거나 codec·verifier가 바뀌어 거부된 것은 손상이 아니다. 이런 거부는 더 작은 옛 revision만 통과시켜 설정 변경이 세션 기록을 버리게 만들므로 폴백하지 않는다. 후보도 같은 규칙을 따른다. 손상된 후보는 건너뛰고, 다른 이유로 거부되거나 런타임과 호환되지 않는 후보에서는 멈춘다. `locked`에서는 versions_held로 커밋되고 지금도 모든 version에 legal hold가 걸린 revision만 후보다. 이 조건을 못 채운 후보는 손상이 아니라 거부로 보고 거기서 멈춘다. hold가 풀린 revision 아래로 내려가면 같은 이유로 더 많은 상태를 잃기 때문이다. held version은 바뀌거나 사라질 수 없으므로 `locked`에서는 후보의 손상도 거부로 본다. 그래서 `locked`의 폴백은 한 칸만 내려간다. 한 attempt는 현재 pointer에 대해 받은 복원 base(pointer든 이전 revision이든)에 묶이고, 같은 attempt가 다른 base를 받으려 하면 거부된다. 묶임은 그 attempt가 다음 checkpoint를 커밋하면 풀린다. 그때부터 attempt의 상태를 설명하는 것은 새 pointer이기 때문이다. 폴백은 조용히 일어나지 않는다. 복원 계획의 `fallback`, 세션의 `durability.checkpoint_fallback_revision`, 이벤트 스트림의 `checkpoint_restore_fallback` system 이벤트가 이를 드러낸다. pointer는 되감지 않는다. 다음 checkpoint는 pointer+1로 커밋되고 그때 fallback 표시가 지워진다. 그동안 pause와 운영 복구의 커버리지는 pointer가 아니라 실제로 복원한 revision 기준으로 판정한다.

**ready 전에 끝난 복원은 한도와 backoff 뒤에 멈춘다 (94S-345, 2026-09-24).** 폴백은 control plane이 손상을 본 경우만 다룬다. 워커 쪽에서만 복원이 실패하는 경우도 있다. 전송 중 손상이 그 예로, 이때 AWS SDK의 응답 checksum이 먼저 거절한다. 이런 실패가 execution 종료로만 처리되면 세션은 곧바로 다시 신호되고, 다음 워커도 같은 이유로 실패하며 generation만 늘어난다.
- **판정**: claim이 복원을 넘긴 attempt(`sessions.restore_attempt_id`)가 `/worker/ready`를 보고하기 전에 execution이 사라지면 복원 실패 1회다. 판정은 `confirmExecutionGone`에서 한다. release 없이 죽은 워커(OOM, SIGKILL)도 같이 센다. 대상은 `active` 세션이다. `resuming`은 자기 한도(`RESUME_LAUNCH_LIMIT`)를 쓴다.
- **backoff**: 3회(`RESTORE_FAILURE_LIMIT`) 전까지는 94S-207의 backoff(30초, 60초)를 `restore_retry_at`에 DB 시계로 기록한다. 그동안 demand, 예약, claim이 모두 세션을 건너뛴다.
- **한도**: 3회째에는 세션이 `recovery_required`·`failed`가 되고 dispatch 신호가 지워진다. queued 입력은 남는다. 계속하는 길은 운영자 결정 `retry_restore`, `start_fresh`, `close`다.
- **재시도 (94S-348)**: 원인이 세션 밖에 있었다면(프록시의 전송 중 손상, 저장소 경로 설정 오류) 운영자가 그것을 고친 뒤 `retry_restore`로 같은 checkpoint를 다시 복원하게 한다. pointer와 폴백 base는 그대로 두고, 횟수·backoff·사유를 지우고, 세션을 `active`로 돌리고, queued 입력을 다시 신호한다. 다음 claim은 실패하던 claim과 같은 revision을 받고, 다시 실패하면 1회부터 센다. 이 결정은 복원 실패가 한도에 닿아 멈춘 세션에만 받는다(`recovery_required`, `restore_failure_count` ≥ 한도, 복원 지점 있음). backoff 중인 세션, 복원 없는 `STARTUP_FAILED`, context gap, unknown turn으로 멈춘 세션에는 409 `REQUEST_STALE`로 거절한다. 같은 checkpoint를 다시 복원해서는 이런 상태가 풀리지 않기 때문이다.
- **초기화**: ready가 넘겨받은 복원 base의 revision을 보고하면 횟수가 0으로 돌아간다. `start_fresh`와 `retry_restore`도 0으로 돌린다.
- **보이는 곳**: 매 실패마다 system `checkpoint_restore_failed` 이벤트(`failures`, `limit`, `reason`, `retry_at`)가 남는다. 한도에 닿으면 status 이벤트(`reason: restore_failed`)도 남는다. 세션 상세의 attention `RESTORE_FAILED`에는 마지막 사유와 다음 시도 시각이 보인다. 사유는 워커가 release에 적은 오류다. 워커는 비밀값을 가리고, 서버는 첫 줄 500자만 저장한다. release 없이 끝났으면 사유는 `execution_gone`이다.
- **복원 없는 claim (94S-347, 94S-302)**: checkpoint가 없는 세션의 worker도 workspace 준비(git clone·fetch)나 엔진 기동에서 같은 식으로 계속 실패할 수 있다. 저장소가 사라졌거나 자격 증명이 거부된 경우다. 그래서 claim은 복원 여부와 상관없이 모두 `restore_attempt_id`에 자기 attempt를 적고, 같은 횟수·backoff·한도를 탄다. 판정 신호는 그 attempt의 첫 `nextInput`이다. 무엇을 받든 부르기만 하면 준비된 것으로 보고 횟수를 0으로 돌린다. worker는 시작과 ready가 끝난 뒤에만 입력을 청하고, 옛 worker도 그렇다. 그래서 fresh claim에 ready를 보내지 않는 옛 worker와 섞여도 idle 종료를 실패로 세지 않는다. 첫 turn 이후의 종료도 세지 않는다. 열린 turn을 남긴 종료는 전처럼 unknown turn의 `recovery_required`로 간다. pause, terminate, close, revocation, resume은 세션을 `active` 밖으로 옮기므로 세지 않는다. 이런 세션의 attention은 `STARTUP_FAILED`이고 모양은 `RESTORE_FAILED`와 같다. 이벤트는 system `startup_failed`이고 `checkpoint_revision`이 없다. 한도에 닿을 때의 status reason은 `startup_failed`다. 어느 쪽인지는 세션에 복원 지점이 있는지로 가른다. checkpoint는 입력을 받은 worker만 커밋할 수 있기 때문이다. 원인을 고친 뒤 `start_fresh`를 쓰면 다시 띄운다. 이런 세션은 버릴 checkpoint가 없다.
- **손상 분류**: SDK의 응답 checksum 불일치는 저장소 어댑터가 `ObjectIntegrityError`로 바꾼다. 워커는 이를 digest 불일치와 같은 `RestoreRefused(CHECKPOINT_UNAVAILABLE)`로 분류한다. SDK는 이 오류를 메시지로만 알린다. 그래서 실제 SDK client로 이 경로를 도는 테스트가 SDK 쪽 변경을 잡는다.
