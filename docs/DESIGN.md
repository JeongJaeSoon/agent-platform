...
- **한도**: 3회째에는 세션이 `recovery_required`·`failed`가 되고 dispatch 신호가 지워진다. queued 입력은 남는다. 계속하는 길은 운영자 결정 `start_fresh`와 `close`이고, context gap과 같다.
- **초기화**: ready가 넘겨받은 복원 base의 revision을 보고하면 횟수가 0으로 돌아간다. `start_fresh`도 0으로 돌린다.
