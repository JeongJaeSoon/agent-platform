...
스크립트는 `rc.json`의 제품 SHA가 RC4 `40864efa`인지 확인하고 이미지 ID를 현재 고정 tag뿐 아니라 실행 중인 API·egress proxy·worker 컨테이너와 대조한다. `SOAK_API_URL`도 해당 compose API의 loopback 포트인지 확인한다. admission-active 세션 10개와 각 세션의 실행 중 worker가 정확히 하나씩인 상태에서 즉시 `p2-phase.json`에 전체 계획표를 기록하고 15분 이상 warmup하며, 첫 표본 직전에도 같은 10-session workload를 다시 확인한다. 이후 UTC 5분 경계에 맞춘 연속 24개 cycle에서 매분 한 번씩 총 120개의 `POST /v1/sessions` 요청을 보낸다. 각 요청은 재시도하거나 다른 표본으로 대체하지 않으며, 실제 전송 시각이 예약된 5초 창을 벗어나거나 201/202가 아니거나 표본·bucket 수가 맞지 않거나 nearest-rank p95가 500ms를 넘으면 `INVALID`다. 끝에서 제품 이미지와 원래 10-session workload를 다시 확인한다. 최종 JSON에는 계획표, 전체 표본, 60개 bucket별 개수, p95, 판정, provenance와 workload 확인값, 500ms 고정 slot의 진단 전용 host probe가 들어가며 `p2-phase.md`가 같은 내용을 요약한다.
