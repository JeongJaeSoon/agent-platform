...
**한 push에 합쳐진 커밋은 누락이 아니다(94S-421).** 두 PR이 1초 안에 squash 머지되면 GitHub는 main ref 갱신을 한 번만 기록한다(`654a3388 → 95b0e7b5`). 가운데 커밋(`1583f5d`, `6e7badf`)은 tip이 된 적이 없어 push 이벤트도 run도 없다. 그 변경은 합쳐진 push의 run이 자기 트리에서 spikes까지 돌린다. 그래서 run이 없는 커밋이 나오면 main ref activity(`repos/{repo}/activity?ref=refs/heads/main`)를 한 번 읽어 둘을 가린다.

- 커밋이 어떤 갱신의 `before`나 `after`로 나오면 tip이었던 것이므로 `missing`이다. `70139eb`은 다음 갱신이 그 커밋에서 출발했다.
- 나오지 않으면 부모를 따라 올라간다. 처음 만나는 tip이 fast-forward 갱신(`push`·`pr_merge`·`merge_queue_merge`)의 `before`이면 `coalesced <커밋> <그 갱신의 after>`로 적고 이슈를 열지 않는다.
- activity에 흔적이 없거나(한 달치만 읽는다), force push가 건너뛰었거나, 10단계 안에 tip에 닿지 않으면 `missing`으로 남긴다.
