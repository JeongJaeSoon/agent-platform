...
- **언제 도는가:** 매일 03:17 KST(`schedule`, main), 수동 실행(`gh workflow run 'D2 gate' --ref <branch>`), 그리고 gate 자신(`scripts/d2-gate/`, `tests/d2-gate*`, 이 workflow)을 바꾼 pull request.
- **무엇을 하는가:** runner의 Docker daemon에서 `scripts/d2-gate/run.sh`를 돈다. `D2_GATE_COMPOSE_OVERRIDE`로 `tests/e2e/compose.ci-mirror.yml`과 `scripts/d2-gate/compose.ci-mirror.yml`을 얹어 서드파티 이미지를 모두 미러에서 받는다. 끝나면 `.github/scripts/d2-gate-verdict.ts`가 테스트마다 이름과 결과(PASS·FAIL·SKIP·MISSING)를 로그와 run summary에 찍고, A–E의 기준표(`report.md`)도 summary에 붙인다. CI의 bun reporter는 통과한 테스트 이름을 찍지 않기 때문이다. 검사 대상은 ci.yml이 d2-gate 테스트로 선언한 skip 목록이다. 그중 하나라도 PASS가 아니면(skip됐거나 돌지 않았으면) gate가 실패한다. build·compose·worker·suite 로그는 `d2-gate-record` artifact에 있다. run의 API key가 든 `vars.sh`는 올리지 않는다.
- **실패하면:** schedule이나 수동 실행에서 job이 `success`로 끝나지 않으면(gate step은 50분에서 끊겨 실패로 남는다. job timeout으로 취소되면 이슈 step이 돌지 않기 때문이다) label `ci-d2-gate-failure`로 이슈를 연다. 이미 열린 이슈가 있으면 거기에 코멘트를 붙인다(`spikes`와 같은 upsert). pull request에서는 빨간 check가 곧 신호이므로 이슈를 열지 않는다.
