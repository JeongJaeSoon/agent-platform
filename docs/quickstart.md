...
로컬 스택은 머신마다 하나로 쓴다. project 이름 `agent-platform`, 0장의 포트, worker label `agent-platform.installation=local`이 기본값이다. `local.sh`는 이 값을 스스로 정하지 않고 `COMPOSE_PROJECT_NAME`·`COMPOSE_FILE`·`EXECUTION_INSTALLATION_ID`를 compose가 해석한 그대로의 스택(`docker compose config`)을 따른다. `up`은 그 스택이 여는 포트를 점검하고 `docker compose port api 3000`이 알려 주는 주소의 `/readyz`를 기다린다. `down`은 그 스택의 `EXECUTION_INSTALLATION_ID` label이 붙은 worker 자원을 지운다. compose의 포트는 `infra/`에 고정돼 있어서, project만 바꾼 두 번째 스택은 `up`의 포트 점검에서 멈춘다. 다른 스택 옆에서 돌려야 하면 자기 project와 임시 포트로 뜨는 `tests/e2e/run.sh`를 쓴다(4장).
...
`down`은 스택과 **모든 데이터**(세션, checkpoint, API key, Gitea 저장소)를 지운다. scheduler가 만든 worker 컨테이너·네트워크·workspace volume은 compose 소유가 아니므로 `agent-platform.installation=<EXECUTION_INSTALLATION_ID>` label(기본 `local`)로 찾아 함께 지운다. 처음부터 다시 하려면 `scripts/local.sh reset`(= `down` 뒤 `up`)을 쓴다. 새 스택에서는 1장의 `KEY=…`부터 다시 한다.
