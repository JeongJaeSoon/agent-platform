...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 27, total: 28 });
      ...
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 27, total: 28 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 28 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("28");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 24, total: 28 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 28 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 28, total: 28 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 28 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
