...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 22, total: 23 });
      ...
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 22, total: 23 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 23 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("23");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 19, total: 23 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 23 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 23, total: 23 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 23 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
