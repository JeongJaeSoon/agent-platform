...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 17, total: 18 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 18 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 17, total: 18 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 18 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("18");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 14, total: 18 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 18 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 18, total: 18 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 18 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
