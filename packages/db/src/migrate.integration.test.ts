...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 21, total: 22 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 22 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 21, total: 22 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 22 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("22");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 18, total: 22 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 22 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 22, total: 22 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 22 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
