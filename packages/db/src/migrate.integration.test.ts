...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 25, total: 26 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 26 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 25, total: 26 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 26 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("26");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 22, total: 26 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 26 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 26, total: 26 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 26 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
