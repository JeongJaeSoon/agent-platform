...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 32, total: 33 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 33 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 32, total: 33 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 33 },
        },
      ]);
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 29, total: 33 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 33 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 33, total: 33 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 33 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
