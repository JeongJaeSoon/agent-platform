...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 14, total: 15 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 15 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 14, total: 15 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 15 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("15");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 11, total: 15 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 15 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 15, total: 15 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 15 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
