...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 19, total: 20 });
      ...
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 19, total: 20 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 20 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("20");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 16, total: 20 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 20 });
