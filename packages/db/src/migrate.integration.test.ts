...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 20, total: 21 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 21 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 20, total: 21 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 21 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("21");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 17, total: 21 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 21 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 21, total: 21 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 21 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
