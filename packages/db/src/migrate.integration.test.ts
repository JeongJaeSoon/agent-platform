...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 29, total: 30 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 30 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 29, total: 30 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 30 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("30");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 26, total: 30 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 30 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 30, total: 30 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 30 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
