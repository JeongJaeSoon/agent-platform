...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 34, total: 35 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 35 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 34, total: 35 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 35 },
        },
      ]);
      ...
      try {
        ...
        expect(journal.rows[0]?.count).toBe("35");
      } finally {
        await verified.end();
      }
    } finally {
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 31, total: 35 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 35 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 35, total: 35 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 35 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
