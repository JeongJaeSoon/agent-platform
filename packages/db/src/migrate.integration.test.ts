...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      expect(first).toEqual({ adopted: 1, applied: 6, total: 7 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 7 });
      expect(
      ...
      ).toEqual([
        {
          ...
          fields: { adopted: 1, applied: 6, total: 7 },
        },
        {
          level: "info",
          message: "db.migrate.noop",
          fields: { adopted: 0, applied: 0, total: 7 },
        },
      ]);
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 4, applied: 3, total: 7 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 7 });
...
integrationTest(
  ...
  async () => {
    ...
    try {
      ...
      const first = await migrateDatabase(testUrl.toString(), { logger });
      const second = await migrateDatabase(testUrl.toString(), { logger });
      expect(first).toEqual({ adopted: 0, applied: 7, total: 7 });
      expect(second).toEqual({ adopted: 0, applied: 0, total: 7 });
      expect(sink.records.map(({ message }) => message)).toEqual([
        "db.migrate.applied",
        "db.migrate.noop",
